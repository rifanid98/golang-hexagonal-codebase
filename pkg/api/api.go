...
type HttpDoer interface {
	HttpDo(param HttpParam) (*http.Response, error)
}
...
// HttpDo HttpDo
func (httpParam *HttpParam) HttpDo(param HttpParam) (*http.Response, error) {
	headers := makeHeader(param.Header)
	timeout := time.Duration(param.Timeout) * time.Second
	switch param.Method {
	case "get":
		return get(param.Url, headers, timeout)
	case "post":
		return post(param.Url, strings.NewReader(param.Body), headers, timeout)
	case "put":
		return put(param.Url, strings.NewReader(param.Body), headers, timeout)
	case "patch":
		return patch(param.Url, strings.NewReader(param.Body), headers, timeout)
	case "delete":
		return delete(param.Url, headers, timeout)
	default:
		return post(param.Url, strings.NewReader(param.Body), headers, timeout)
	}
}
